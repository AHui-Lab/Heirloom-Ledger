...
const AUCTION_TEMPLATE_IDS := ["auctionfan", "auctionvase", "tea_caddy", "seal", "inkstone", "painting", "burner", "plate"]
...
func auction_preview() -> Dictionary:
	...
	var lot_count := 6 + day % 3
	var lots: Array = []
	var rivals: Array = revision_content.get("auction_rivals", [])
	for index in range(lot_count):
		var template_id: String = AUCTION_TEMPLATE_IDS[(day + index) % AUCTION_TEMPLATE_IDS.size()]
		var item: Dictionary = all_items[template_id].duplicate(true)
		item["id"] = "auction_%d_%d" % [day, index]
		item["title"] = item["title"].trim_prefix("小拍")
		item["location"] = "unseen"
		item["cost"] = 0
		item["public_provenance"] = ""
		item["source_claim"] = "委托方只提供了本场图录中的简略说明，具体依据需要在预展中自行核对。"
		item["provenance"] = item["source_claim"]
		item["clue_rules"] = item["clues"].duplicate(true)
		item["clues"] = []
		item["revealed_clues"] = []
		...
		var opening := maxi(200, int(item["market_value"] * (0.28 + float(index % 3) * 0.04)))
		var rival_limit := maxi(opening + 200, int(item["market_value"] * (0.62 + float((day + index) % 4) * 0.10)))
		var visible_rivals: Array = []
		if not rivals.is_empty():
			var visible_count := 2 + (day + index) % 3
			for rival_index in range(visible_count):
				visible_rivals.append(rivals[(index + rival_index) % rivals.size()])
		lots.append({
			"item_id": item["id"], "lot_number": index + 1, "price": opening,
			"rival_limit": rival_limit, "status": "upcoming", "marked": false,
			"outcome": "尚未开拍", "rivals": visible_rivals
		})
	auction = {"lots": lots, "current_index": 0, "phase": "preview", "done": false, "outcome": "预展", "fee_percent": 5}
	return auction

func auction_lots() -> Array:
	var sale := auction_preview()
	return sale.get("lots", []) if not sale.is_empty() else []

func mark_auction_lot(item_id: String) -> bool:
	if auction.is_empty() or auction.get("phase", "") != "preview": return false
	for lot in auction["lots"]:
		if lot["item_id"] == item_id:
			lot["marked"] = not bool(lot["marked"])
			return true
	return false
...
func start_auction() -> bool:
	...
	if not auction["lots"].is_empty(): auction["lots"][0]["status"] = "active"
	return true

func current_auction_lot() -> Dictionary:
	if auction.is_empty() or auction.get("phase", "") != "bidding": return {}
	var index := int(auction.get("current_index", 0))
	if index < 0 or index >= auction.get("lots", []).size(): return {}
	return auction["lots"][index]

func _advance_auction_lot() -> void:
	var next := int(auction.get("current_index", 0)) + 1
	auction["current_index"] = next
	if next >= auction["lots"].size():
		auction["phase"] = "finished"
		auction["done"] = true
		auction["outcome"] = "本场竞价结束"
	else:
		auction["lots"][next]["status"] = "active"

func skip_auction_lot() -> bool:
	var lot := current_auction_lot()
	if lot.is_empty(): return false
	lot["status"] = "passed"
	lot["outcome"] = "你没有举牌"
	_advance_auction_lot()
	return true

func finish_auction_early() -> void:
	if auction.is_empty() or auction.get("done", false): return
	var index := int(auction.get("current_index", 0))
	for cursor in range(index, auction.get("lots", []).size()):
		if auction["lots"][cursor]["status"] in ["upcoming", "active"]:
			auction["lots"][cursor]["status"] = "passed"
			auction["lots"][cursor]["outcome"] = "提前离场"
	auction["phase"] = "finished"
	auction["done"] = true
	auction["outcome"] = "提前离场，未处理的拍品均未成交"

func bid(amount: int) -> String:
	if not auction.has("lots"): return super.bid(amount)
	var lot := current_auction_lot()
	if lot.is_empty(): return "当前没有正在竞价的拍品。"
	var item: Dictionary = all_items[lot["item_id"]]
	if amount <= int(lot["price"]): return "新报价必须高于当前价。"
	var total := amount + int(ceil(amount * 0.05))
	if total > money: return "含5%费用共¥%d，资金不足。" % total
	if amount <= int(lot["rival_limit"]):
		lot["price"] = amount + 100
		lot["outcome"] = "场内有人跟到¥%d" % int(lot["price"])
		return "%s。你可以继续举牌，也可以放弃这一件。" % lot["outcome"]
	lot["price"] = amount
	lot["status"] = "won"
	lot["outcome"] = "落槌成交"
	_acquire(item, total, "槐安小拍")
	_remember("chen", "第%d日参与小拍，拍得《%s》。" % [day, item["title"]], 1)
	_advance_auction_lot()
	return "落槌¥%d，加5%%费用共¥%d。《%s》进入库存。" % [amount, total, item["title"]]

func leave_auction() -> void:
	if auction.has("lots"): finish_auction_early()
	else: super.leave_auction()

func inspect_outside(item_id: String) -> Dictionary:
	if auction.has("lots"):
		for lot in auction["lots"]:
			if str(lot["item_id"]) == item_id and lot["status"] not in ["won"]:
				var item: Dictionary = all_items.get(item_id, {})
				if item.is_empty(): return {}
				item["public_provenance"] = "拍卖图录说明（未独立核实）：%s" % item["source_claim"]
				return _appraise_item(item, "拍卖预展看货")
	return super.inspect_outside(item_id)
